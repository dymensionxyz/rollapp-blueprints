...
func validatePKIToken(attestationToken string) (jwt.Token, error) {
	...
	rawRootCertificate, err := os.ReadFile(rootCertificateFile)
	if err != nil {
...
func validateClaimsAgainstOPAPolicy(token jwt.Token, ekm string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to read OPA validation values: %w", err)
	}
...
func evaluateOPAPolicy(ctx context.Context, token jwt.Token, ekm string, policyData string) (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to unmarshal policy data: %w", err)
	}
...
func main() {
	...
	if ipAddress == "" {
		log.Fatalf("Error: Environment variable %s is not set.", ipAddrEnvVar)
	}
	...
	for {
		...
		if err != nil {
			...
			fmt.Println("Waiting 10 seconds before retrying...")
			time.Sleep(10 * time.Second)
			continue // Try dialing again in the next iteration
		}

		// Defer closing the connection within the loop
		func() {
			defer conn.Close()
			...
			if err != nil {
				...
				return // Continue to the next iteration after delay
			}
			...
			fmt.Println("Validated token and claims.")
			fmt.Println("Closing the connection")
		}() // End of anonymous function for defer conn.Close()

		fmt.Println("Waiting 10 seconds before next check...")
		time.Sleep(10 * time.Second)
	}
}
