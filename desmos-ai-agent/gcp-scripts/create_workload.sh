...
create_workload() {
  ...
  echo "s/PROJECT_ID/$project_id/"
  echo "$PARENT_DIR"/src/"$folder_name"/workload.go
  echo "s/PROJECT_NUMBER/$PROJECT_NUMBER/"
  ...
  cd "$PARENT_DIR"/src/"$folder_name" || exit
  docker build . -t "$IMAGE_REFERENCE"
  cd "$PARENT_DIR"/scripts || exit
